<div>
    ...
    @isset($comments)
        @foreach($comments as $comment)
            ...
            <div class="row">
                @can('update', $comment)
                    <button type="button" class="btn btn-primary btn-sm" data-edit-comment-id="{{ $comment->id }}">Редактировать
                    </button>
                @endcan
                @can('delete', $comment)
                    <form
            ...
            @can('update', $comment)
                <form action="{{ route('update_comment', ['comment' => $comment->id]) }}"
                      method="POST"
                      style="display: none"
                ...
                >
                    ...
                    <textarea class="form-control" name="commentText"
                              rows="8">{{ $comment->comment }}</textarea>
                    </div>
